...
public class VoldemortClient extends DB
{
    ...
    public void init() throws DBException
    {
        ...
        ClientConfig clientConfig = new ClientConfig( Utils.mergeMapToProperties( getProperties(), properties, true ) );
        socketFactory = new SocketStoreClientFactory( clientConfig );

        // Retrieve store name
        storeName = Utils.mapGetDefault( getProperties(), "store_name", "usertable" );

        // Use store name to retrieve client
        storeClient = socketFactory.getStoreClient( storeName );
        if ( storeClient == null ) throw new DBException( "Unable to instantiate store client" );
    ...
    public int read( String table, String key, Set<String> fields, Map<String, ByteIterator> result )
    {
        ...
        if ( versionedValue == null ) return NOT_FOUND;
        ...
        if ( fields != null )
        {
            for ( String field : fields )
            {
                ...
                if ( val != null ) result.put( field, new StringByteIterator( val ) );
            }
        }
        else
        {
