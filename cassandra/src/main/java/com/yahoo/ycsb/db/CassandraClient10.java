...
public class CassandraClient10 extends DB
{
    ...
    public void init() throws DBException
    {
        ...
        column_family = Utils.mapGetDefault( getProperties(), COLUMN_FAMILY_PROPERTY, COLUMN_FAMILY_PROPERTY_DEFAULT );
        parent = new ColumnParent( column_family );

        ConnectionRetries = Integer.parseInt( Utils.mapGetDefault( getProperties(), CONNECTION_RETRY_PROPERTY,
                CONNECTION_RETRY_PROPERTY_DEFAULT ) );

        OperationRetries = Integer.parseInt( Utils.mapGetDefault( getProperties(), OPERATION_RETRY_PROPERTY,
                OPERATION_RETRY_PROPERTY_DEFAULT ) );

        String username = getProperties().get( USERNAME_PROPERTY );
        String password = getProperties().get( PASSWORD_PROPERTY );

        readConsistencyLevel = ConsistencyLevel.valueOf( Utils.mapGetDefault( getProperties(),
                READ_CONSISTENCY_LEVEL_PROPERTY, READ_CONSISTENCY_LEVEL_PROPERTY_DEFAULT ) );
        writeConsistencyLevel = ConsistencyLevel.valueOf( Utils.mapGetDefault( getProperties(),
                WRITE_CONSISTENCY_LEVEL_PROPERTY, WRITE_CONSISTENCY_LEVEL_PROPERTY_DEFAULT ) );
        scanConsistencyLevel = ConsistencyLevel.valueOf( Utils.mapGetDefault( getProperties(),
                SCAN_CONSISTENCY_LEVEL_PROPERTY, SCAN_CONSISTENCY_LEVEL_PROPERTY_DEFAULT ) );
        deleteConsistencyLevel = ConsistencyLevel.valueOf( Utils.mapGetDefault( getProperties(),
                DELETE_CONSISTENCY_LEVEL_PROPERTY, DELETE_CONSISTENCY_LEVEL_PROPERTY_DEFAULT ) );

        _debug = Boolean.parseBoolean( Utils.mapGetDefault( getProperties(), "debug", "false" ) );
    ...
    public int read( String table, String key, Set<String> fields, Map<String, ByteIterator> result )
    {
        ...
        for ( int i = 0; i < OperationRetries; i++ )
        {
            ...
            try
            {
                ...
                if ( fields == null )
                {
                    predicate = new SlicePredicate().setSlice_range( new SliceRange( emptyByteBuffer, emptyByteBuffer,
                            false, 1000000 ) );
            ...
            Vector<Map<String, ByteIterator>> result )
    {
        ...
        for ( int i = 0; i < OperationRetries; i++ )
        {
            ...
            try
            {
                ...
                KeyRange kr = new KeyRange().setStart_key( startkey.getBytes( "UTF-8" ) ).setEnd_key( new byte[] {} ).setCount(
                        recordcount );
