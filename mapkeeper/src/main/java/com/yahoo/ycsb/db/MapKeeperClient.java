...
import com.ldbc.workloads.CoreWorkloadProperties;
...
public class MapKeeperClient extends DB
{
    ...
    ByteBuffer encode( Map<String, ByteIterator> values )
    {
        ...
        for ( String k : values.keySet() )
        {
            ...
            array[i] = '\t'; // XXX would like to use sane delimiter (null, 254,
                             // 255, ...) but java makes this nearly impossible
            i++;
            ...
            Vector<Map<String, ByteIterator>> result )
    {
        try
        {
            ...
            RecordListResponse res = c.scan( table, ScanOrder.Ascending, bufStr( startkey ), true, null, false,
                    recordcount, 0 );
    ...
    public int insert( String table, String key, Map<String, ByteIterator> values )
    {
        try
        {
            int ret = ycsbThriftRet( c.insert( table, bufStr( key ), encode( values ) ), ResponseCode.Success,
