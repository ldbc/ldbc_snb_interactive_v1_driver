...
public class SimpleSummaryWorkloadMetricsFormatter implements WorkloadMetricsFormatter {
    ...
    private String formatOneMetricRuntime(String offset, OperationMetricsSnapshot metric, int namePadRightDistance, int countPadRightDistance) {
        String name = (null == metric.name()) ? DEFAULT_NAME : metric.name();
        String unit = (null == metric.durationUnit()) ? DEFAULT_UNIT : TEMPORAL_UTIL.abbreviatedTimeUnit(metric.durationUnit());
        StringBuilder sb = new StringBuilder();
        sb.append(offset);
        sb.append(String.format("%1$-" + namePadRightDistance + "s", name));
        sb.append("Count: ").append(String.format("%1$-" + countPadRightDistance + "s", INTEGER_FORMATTER.format(metric.runTimeMetric().count()))).append(" ");
        sb.append("Mean: ").append(FLOAT_FORMATTER.format(metric.runTimeMetric().mean())).append(" ").append(unit).append("\n");
        return sb.toString();
    }
}
