...
public class Query6EventStreamReader_OLD implements Iterator<Operation<?>> {
    ...
    public static final CsvEventStreamReader_OLD.EventDecoder<Operation<?>> EVENT_DECODER = new CsvEventStreamReader_OLD.EventDecoder<Operation<?>>() {
        ...
        public Operation<?> decodeEvent(String[] csvRow) {
            ...
            String tagName = params.get(TAG_NAME).asText();
            return new LdbcQuery6(personId, personUri, tagName, LdbcQuery6.DEFAULT_LIMIT);
        }
    };

    public Query6EventStreamReader_OLD(Iterator<String[]> csvRowIterator) {
        this(csvRowIterator, CsvEventStreamReader_OLD.EventReturnPolicy.AT_LEAST_ONE_MATCH);
    }

    public Query6EventStreamReader_OLD(Iterator<String[]> csvRowIterator, CsvEventStreamReader_OLD.EventReturnPolicy eventReturnPolicy) {
