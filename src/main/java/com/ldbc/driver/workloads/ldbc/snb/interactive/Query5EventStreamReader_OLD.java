...
public class Query5EventStreamReader_OLD implements Iterator<Operation<?>> {
    ...
    public static final CsvEventStreamReader_OLD.EventDecoder<Operation<?>> EVENT_DECODER = new CsvEventStreamReader_OLD.EventDecoder<Operation<?>>() {
        ...
        public Operation<?> decodeEvent(String[] csvRow) {
            ...
            Date joinDate;
            try {
                joinDate = DATE_FORMAT.parse(dateString);
            } catch (ParseException e) {
                throw new GeneratorException(String.format("Error parsing date string\n%s", dateString), e);
            }
            return new LdbcQuery5(personId, personUri, joinDate, LdbcQuery5.DEFAULT_LIMIT);
        }
    };

    public Query5EventStreamReader_OLD(Iterator<String[]> csvRowIterator) {
        this(csvRowIterator, CsvEventStreamReader_OLD.EventReturnPolicy.AT_LEAST_ONE_MATCH);
    }

    public Query5EventStreamReader_OLD(Iterator<String[]> csvRowIterator, CsvEventStreamReader_OLD.EventReturnPolicy eventReturnPolicy) {
