package com.ldbc.driver.util.neo_csv;
...
public class QuoteAwareCharSeeker implements CharSeeker {
    ...
    public boolean seek(Mark mark, int[] untilOneOfChars) throws IOException {
        if (actual.seek(mark, withQuoteChars(untilOneOfChars))) {
            ...
            if (mark.isEndOfLine() || mark.character() != quotationChar) {   // Good, we're there. Return the value
            ...
            startPosition++;
            while (actual.seek(mark, quotationCharDelimiter) && mark.isEndOfLine()) {   // Keep on looking for the end of that quote, since the seeker returns line endings
                // no matter what we give it
            }

            if (mark.isEndOfLine()) {   // We didn't find the other quote
