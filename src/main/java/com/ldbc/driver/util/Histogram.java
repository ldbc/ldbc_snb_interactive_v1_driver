...
public class Histogram<THING_TYPE, COUNT extends Number> {
    ...
    public static <THING_TYPES, COUNTS extends Number> boolean equalsWithinTolerance(Histogram<THING_TYPES, COUNTS> first, Histogram<THING_TYPES, COUNTS> second, Number tolerance) {
        if (first == second) return true;
        if (first == null) return false;
        if (second == null) return false;
        if (first.valuedBuckets == null) {
            if (second.valuedBuckets != null) return false;
        }
        return doEqualsWithinTolerance(first, second, tolerance);
    }

    private static <THING_TYPES, COUNTS extends Number> boolean doEqualsWithinTolerance(Histogram<THING_TYPES, COUNTS> first, Histogram<THING_TYPES, COUNTS> second, Number tolerance) {
